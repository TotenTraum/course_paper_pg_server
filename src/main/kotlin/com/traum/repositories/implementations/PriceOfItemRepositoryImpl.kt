...
class PriceOfItemRepositoryImpl(private val connection: Connection) : IPriceOfItemRepository {
    companion object {
        private const val SELECT_PRICE_BY_ID =
            """select "Id", "ItemId", "Price", "DateOfChange" from "PricesOfItem" where "Id" = ?"""
        private const val SELECT_PRICE_BY_ITEM_ID =
            """select "Id", "ItemId", "Price", "DateOfChange" from "PricesOfItem" where "ItemId" = ?"""
        private const val SELECT_PRICE_BY_ID_ORDERED =
            """select "Id", "ItemId", "Price", "DateOfChange" from "PricesOfItem" where "ItemId" = ? order by "DateOfChange" desc  limit 1"""
        private const val INSERT_PRICE = "{? = call add_price_of_item(?, ?)}"
    }

    override suspend fun getAllByItemId(itemId: Long): List<PriceOfItem> = withContext(Dispatchers.IO) {
        val statement =
            connection.prepareStatement(SELECT_PRICE_BY_ITEM_ID)
    ...
    override suspend fun getById(id: Long): PriceOfItem = withContext(Dispatchers.IO) {
        val statement =
